...
public abstract class DicDataReader : MonoBehaviour
{
    ...
    public static bool IsDicStateExist(string DicName, string objName)
    {

        if (DicName == SceneObjDataDicName)
        {
            return SceneObjDataManager.dataManagerInstance.CheckKeyExistInObjDataDictionary(objName);
        }
        else
        {
            return false;
