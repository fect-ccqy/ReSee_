...
public class UIPropList : MonoBehaviour,TaskMessenger,TaskProcessor<PropContent>
{
    ...
    [SerializeField] private GameObject propTextObj;
    [SerializeField] private Image propTextBackImage;
    ...
    //在道具被选中时，供道具来调用
    public void ShowPropText(string thePropText)
    {
        if (thePropText != "" && thePropText != null)
        {

            propTextObj.SetActive(true);
            propText.text = thePropText;
            LayoutRebuilder.ForceRebuildLayoutImmediate(propText.rectTransform);
            //print(propText.rectTransform.sizeDelta);
           // print(propTextBackImage.rectTransform.sizeDelta);
            propTextBackImage.rectTransform.sizeDelta = propText.rectTransform.sizeDelta + new Vector2(23, 0);
